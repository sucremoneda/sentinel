...
def main():
    ...
    if not is_sucrd_port_open(sucrd):
        print("Cannot connect to sucrd. Please ensure sucrd is running and the JSONRPC port is open to Sentinel.")
        return

    # check sucrd sync
    if not sucrd.is_synced():
        print("sucrd not synced with network! Awaiting full sync before running Sentinel.")
        return
