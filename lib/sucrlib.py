...
def is_valid_sucr_address(address, network='mainnet'):
    ...
    sucr_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Sucr address
        return False

    if (address_version != sucr_version):
        return False
...
# shims 'til we can fix the sucrd side
def SHIM_serialise_for_sucrd(sentinel_hex):
    from models import SUCRD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = SUCRD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in sucrd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # sucrd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    sucrd_hex = serialise(obj)
    return sucrd_hex


# shims 'til we can fix the sucrd side
def SHIM_deserialise_from_sucrd(sucrd_hex):
    from models import SUCRD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(sucrd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('sucr.conf')
