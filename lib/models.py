...
class GovernanceObject(BaseModel):
    ...
    def import_gobject_from_sucrd(self, sucrd, rec):
        ...
        # get/create, then sync payment amounts, etc. from sucrd - Sucrd is the master
        try:
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = SUCRD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            if not sucrlib.is_valid_sucr_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Sucr address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/SucrDrive, etc.)
        return False
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = SUCRD_GOVOBJ_TYPES['superblock']
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = SUCRD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True
